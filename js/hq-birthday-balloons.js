(() => {
  ...
  function paletteFor(name='') {
    if (Object.prototype.hasOwnProperty.call(known,name)) return palettes[known[name]];
    let hash=0; for (const ch of name) hash=(hash*31+ch.charCodeAt(0))>>>0;
    return palettes[hash%palettes.length];
  }

  function balloonSvg(p, compact=false) {
    const [a,b,accent]=p;
  ...
  function ensureStyles() {
    ...
    style.textContent=`
      ...
      .birthday-avatar.hq-balloon-avatar{width:72px;height:78px;border-radius:0;background:transparent;overflow:visible}
  ...
  function enhance() {
    ...
    document.querySelectorAll('.birthday-card').forEach(card=>{
      ...
      if(!/birthday/i.test(meta)) return;
      const p=paletteFor(name);
      card.classList.add('hq-colour');
      card.style.background=p[3];
      const avatar=card.querySelector('.birthday-avatar');
      if(avatar && avatar.dataset.hq!=='1'){
        avatar.classList.add('hq-balloon-avatar');
        avatar.innerHTML=balloonSvg(p);
        avatar.dataset.hq='1';
      }
    });
    ...
    document.querySelectorAll('.home-reminder-card.birthday').forEach(card=>{
      ...
      if(icon && icon.dataset.hq!=='1'){
        icon.classList.add('hq-balloon-home');
        icon.innerHTML=balloonSvg(p,true);
        icon.dataset.hq='1';
      }
    });
  }
