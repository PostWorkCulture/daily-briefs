...
- Validation: 89 existing Python tests and four new Inbox profile/routing/startup tests pass. Private-route allowlist, owner access and upstream credential isolation checked separately in the Inbox project. Responsive CI passed on 390px mobile, 1366px desktop and 1920px widescreen, including Pete/Sofia route isolation. GitHub Pages deployment and owner-only companion deployment both succeeded. Live signed-in Gmail interaction still requires Pete's session.
...
- Reliability: activated an independent daily 06:35 Europe/London freshness monitor outside GitHub Actions so a missing schedule event is detected and recovered rather than waiting for Pete to report it. It stays silent when both profiles are current and reports only when recovery is needed or blocked.
...
- Validation and publication: 72 unit tests and the protected mobile/Chromebook browser gate passed. PR #37 was squash-merged as `0d02d3c`; the contract-enforced refresh committed `b98252d`, Pages deployment #326 passed, and both public routes and JSON feeds returned HTTP 200 with 16 Local News articles, six Career jobs, and no section-type errors.
...
- Scope protected: no section order, route, profile behaviour, morning refresh logic, calendar data, news rules, Arsenal data, or TV selection logic changed. The only content correction was the release-gate world fact repair above.
...
- Validation result: all 33 unit tests pass; static icon, manifest, route-cache, SVG, JavaScript, Python, JSON, and diff-hygiene checks pass; PR #19's remote Chromium suite passes its mobile and desktop assertions, including company-logo decoding and AI's no-photography rule.
- Publication: PR #19 was squash-merged as `5f81959`. GitHub Pages serves every Open Horizon v2 metadata path, and Pete and Sofia profile routes resolve to their locked URLs with `icon=v2`.
- Live verification: Pete's 10 AI cards render six OpenAI marks, three Google Gemini marks, and one Anthropic mark, all decoded successfully. AI contains zero article-photography blocks. Device-level replacement of Samsung's stored old bookmark remains pending Pete deleting and re-adding that bookmark once.
